...
// --Commented out by Inspection STOP (8/10/17, 5:13 PM)
        ...
        return h.sqr(list.get(0)) + h.sqr(list.get(1)) == h.sqr(list.get(2));
    }
...
// --Commented out by Inspection STOP (8/10/17, 5:13 PM)
    ...
    private static String removeDuplicateCharacters() {
        int[] freq = new int[1024];
        char[] chars = "zaabcbd".toCharArray();
        for (char c : chars)
            freq[c]++;
