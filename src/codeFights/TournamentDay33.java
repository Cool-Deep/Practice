...
// --Commented out by Inspection STOP (8/10/17, 5:13 PM)
    ...
    private static int differentValuesInMultiplicationTable() {
        int result = 0;
        for (int value = 1; value <= 3 * 2; value++) {
            for (int i = 1; i <= Math.min(3, 2); i++) {
                if ( value / i <= Math.max(3, 2) && value % i == 0 ) {
                    result++;
                    break;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        // System.out.println(caseUnification("ABa"));
        System.out.println(differentValuesInMultiplicationTable());
    }
